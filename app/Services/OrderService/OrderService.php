...
class OrderService extends CoreService implements OrderServiceInterface
{
    ...
    public function create(array $data): array
    {
        ...
        if (data_get($data, 'table_id')) {
            ...
            if ($order?->table?->waiter?->isWork) {
                $order->update([
                    'waiter_id' => $order->table->waiter->id,
                ]);
            }
...
//			}
    ...
    public function updateDeliveryMan(?int $orderId, int $deliveryman, ?int $shopId = null): array
    {
        try {
            ...
            if (!auth('sanctum')->user()->hasRole('admin') && $user->invitations?->count() > 0 && !$user->invitations?->where('shop_id', $order->shop_id)?->first()?->id) {
...
//            }
            ...
            $order->update([
                'waiter_id' => data_get($waiter,'id',$order->table?->waiter?->id),
            ]);
    ...
    public function attachWaiter(?int $id): array
    {
        try {
            ...
            if ($user?->table_id !== $order?->table_id) {
                return [
                    'status'  => false,
